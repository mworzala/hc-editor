...
- **Commit when work is complete.** Once `typecheck` and `test` are green for a piece of work, create a git commit before considering the task done. Do not push — leave commits local so the user reviews them with `git log`/`git diff` and pushes when ready. Split unrelated fixes into separate commits (one logical change per commit) rather than batching them. Commit messages follow the existing style in `git log` — a `type:` prefix (`fix:`, `feat:`, `chore:`, `refactor:`, …), a one-line subject in the imperative, and a body that explains *why* (not just *what*). Use a HEREDOC for the message so formatting survives the shell.
